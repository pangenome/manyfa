        ...
        futures.push_back(std::async(std::launch::async, [&, file_idx, fasta_file, debug]() {
            try {
                ...
                if (debug) {
                    ...
                    std::cerr << "[manyfa] Opening FASTA file: " << fasta_file << std::endl;
                }
    ...
    for (const auto& file : fasta_files) {
        try {
            ...
            if (debug) {
                ...
                std::cerr << "[manyfa] Loaded reader for: " << file << std::endl;
            }
            ...
            for (const auto& idx : thread_work[t]) {
                ...
                try {
                    ...
                    if (it == sequence_index.end()) {
                        std::lock_guard<std::mutex> lock(output_mutex);
                        std::cerr << "[manyfa] Warning: Sequence '" << entry.chrom << "' not found in any FASTA file - skipping" << std::endl;
                        continue;
                    }
                    ...
                    if (entry.start < 0 || entry.end > source.length || entry.start >= entry.end) {
                        std::lock_guard<std::mutex> lock(output_mutex);
                        std::cerr << "[manyfa] Warning: Invalid coordinates for " << entry.chrom 
